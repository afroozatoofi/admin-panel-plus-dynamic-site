...
class CaptchaComponent extends Component {
	...
	public $settings = array (
			...
			'field' => 'security_code',
			'rotate' => true,
			'reload_txt' => 'بروزرسانی',
			'clabel' => '<p>کد امنیتی بالا را وارد نمایید</p>',
			'mlabel' => '<p>پاسخ محاسبات بالا</p>',
			'class' => 'clabel',
			'action' => 'captcha' 
	);
	 ...
	 *        	A ComponentCollection this component can use
	...
	public function beforeRender(Controller $controller) {
		$controller->helpers ['Captcha'] = array_merge ( $this->__getSettings (), array (
	...
	function __imageCaptcha() {
		...
		if (! $this->__TTFEnabled ()) {
			...
			$height = 30;
			$theme = "default";
			$this->themes [$theme] ['txtcolor'] = array (
					255,
					255,
					255 
			);
	...
	function deleteCode($sessionKey) {
		$sessionKey = str_replace ( '.', '_', $sessionKey );
		$this->Session->delete ( "Captcha.{$sessionKey}" );
	}
	private function __prepareThemes() {
		if ($this->settings ['theme'] == 'random') {
			...
			$this->themes ['random'] ['txtcolor'] = array (
					$txt_r,
					$txt_g,
					$txt_b 
			);
