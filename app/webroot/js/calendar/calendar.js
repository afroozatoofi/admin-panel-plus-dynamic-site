...
Date.prototype.getJalaliMonthDays = function(month) {
	var year = this.getJalaliFullYear();
	if (typeof month == "undefined") {
...
Date.prototype.getLocalMonthDays = function(dateType, month) {
	if (dateType == 'jalali') {
		return this.getJalaliMonthDays(month);
	} else {
...
Date.prototype.getLocalDayOfYear = function(dateType) {
	if (dateType == 'jalali') {
		return this.getJalaliDayOfYear();
	} else {
 ...
 */
Date.prototype.getJalaliWeekNumber = function() {
	...
	if (days < 0) return new Date(this - this.getJalaliDay()*Date.DAY).getJalaliWeekNumber();
...
Date.prototype.getLocalWeekNumber = function(dateType) {
	if (dateType == 'jalali') {
		return this.getJalaliWeekNumber();
	} else {
...
Date.prototype.print = function (str, dateType, useLangNumbers) {
	var m = this.getLocalMonth(false, dateType);
	var d = this.getLocalDate(false, dateType);
	var y = this.getLocalFullYear(false, dateType);
	var wn = this.getLocalWeekNumber(false, dateType);
...
Date.prototype._calSetJalaliFullYear = function(y) {
	...
	if (date.getJalaliMonth() != this.getJalaliMonth()) this.setJalaliDate(29);
...
Date.prototype.setLocalFullYear = function(UTC, dateType, y, m, d) {
	if (dateType == 'jalali') {
		if (m == undefined) m = UTC ? this.getJalaliMonth() : this.getJalaliMonth();
		if (d == undefined) d = UTC ? this.getJalaliDate() : this.getJalaliDate();
 		return UTC ? this.setJalaliFullYear(y, m, d) : this.setJalaliFullYear(y, m, d);
	} else {
		if (m == undefined) m = UTC ? this.getMonth() : this.getMonth();
		if (d == undefined) d = UTC ? this.getDate() : this.getDate();
 		return UTC ? this.setFullYear(y, m, d) : this.setFullYear(y, m, d);
	}
}
...
Date.prototype.setLocalMonth = function(UTC, dateType, m, d) {
	if (dateType == 'jalali') {
		if (d == undefined) d = UTC ? this.getJalaliDate() : this.getJalaliDate();
 		return UTC ? this.setJalaliMonth(m, d) : this.setJalaliMonth(m, d);
	} else {
		if (d == undefined) d = UTC ? this.getDate() : this.getDate();
 		return UTC ? this.setMonth(m, d) : this.setMonth(m, d);
	}
}

Date.prototype.setLocalDate = function(UTC, dateType, d) {
	if (dateType == 'jalali') {
 		return UTC ? this.setJalaliDate(d) : this.setJalaliDate(d);
	} else {
 		return UTC ? this.setDate(d) : this.setDate(d);
	}
}

Date.prototype.getLocalFullYear = function(UTC, dateType) {
	if (dateType == 'jalali') {
 		return UTC ? this.getJalaliFullYear() : this.getJalaliFullYear();
	} else {
 		return UTC ? this.getFullYear() : this.getFullYear();
	}
}

Date.prototype.getLocalMonth = function(UTC, dateType) {
	if (dateType == 'jalali') {
 		return UTC ? this.getJalaliMonth() : this.getJalaliMonth();
	} else {
 		return UTC ? this.getMonth() : this.getMonth();
	}
}

Date.prototype.getLocalDate = function(UTC, dateType) {
	if (dateType == 'jalali') {
 		return UTC ? this.getJalaliDate() : this.getJalaliDate();
	} else {
 		return UTC ? this.getDate() : this.getDate();
	}
}

Date.prototype.getLocalDay = function(UTC, dateType) {
	if (dateType == 'jalali') {
 		return UTC ? this.getJalaliDay() : this.getJalaliDay();
	} else {
 		return UTC ? this.getDay() : this.getDay();
	}
}
